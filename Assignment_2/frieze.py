...
class Frieze:
    ...
    def _is_correct_input(self):
        ...
        for i in self.list_vertical[-1]:
            if i[-2] == '1' or i[-3] == '1' or i[-4] == '1':
                raise FriezeError('Input does not represent a frieze.')
        for i in self.translated_file[0]:
            if i[-1] == '1' or i[-2] == '1':
                raise FriezeError('Input does not represent a frieze.')
        for i in self.translated_file[-1]:
            if i[-4] == '1':
                raise FriezeError('Input does not represent a frieze.')
        if all([self.list_vertical[0][j][-1] == '1' for j in range(1, self.height)]):
            if all([self.list_vertical[-1][j][-1] == '1' for j in range(1, self.height)]):
                raise FriezeError('Input does not represent a frieze.')
    ...
    def is_rotation(self):
        if self.height % 2 == 1:
            axis_horizontal = int(self.height / 2)
            for axis_vertical in range(int(self.period / 2) - 1, int(self.length / 2) + 1):
                flag = 0
                for i in range(-int(self.height / 2), int(self.height / 2) + 2):
                    for j in range(-int(self.period / 2), int(self.period / 2) + 1):
                        if self.west_east_horizontal[axis_horizontal + i][axis_vertical + j] == 1:
                            if self.west_east_horizontal[axis_horizontal - i + 1][axis_vertical - j] != 1:
                                flag = 1
                for i in range(-int(self.height / 2) + 1, int(self.height / 2) + 2):
                    for j in range(-int(self.period / 2), int(self.period / 2) + 2):
                        if self.north_south_horizontal[axis_horizontal + i][axis_vertical + j] == 1:
                            if self.north_south_horizontal[axis_horizontal - i + 2][axis_vertical - j + 1] != 1:
                                flag = 1
                for i in range(-int(self.height / 2) + 1, int(self.height / 2) + 2):
                    for j in range(-int(self.period / 2), int(self.period / 2) + 1):
                        if self.sw_ne_horizontal[axis_horizontal + i][axis_vertical + j] == 1:
                            if self.sw_ne_horizontal[axis_horizontal - i + 2][axis_vertical - j] != 1:
                                flag = 1
                for i in range(-int(self.height / 2), int(self.height / 2) + 1):
                    for j in range(-int(self.period / 2), int(self.period / 2) + 1):
                        if self.nw_se_horizontal[axis_horizontal + i][axis_vertical + j] == 1:
                            if self.nw_se_horizontal[axis_horizontal - i][axis_vertical - j] != 1:
                                flag = 1
                if flag == 0:
                    return True
            for axis_vertical in range(int(self.period / 2) - 1, int(self.length / 2) + 1):
                flag = 0
                for i in range(-int(self.height / 2), int(self.height / 2) + 2):
                    for j in range(-int(self.period / 2), int(self.period / 2)):
                        if self.west_east_horizontal[axis_horizontal + i][axis_vertical + j] == 1:
                            if self.west_east_horizontal[axis_horizontal - i + 1][axis_vertical - j - 1] != 1:
                                flag = 1
                for i in range(-int(self.height / 2) + 1, int(self.height / 2) + 2):
                    for j in range(-int(self.period / 2), int(self.period / 2) + 1):
                        if self.north_south_horizontal[axis_horizontal + i][axis_vertical + j] == 1:
                            if self.north_south_horizontal[axis_horizontal - i + 2][axis_vertical - j] != 1:
                                flag = 1
                for i in range(-int(self.height / 2) + 1, int(self.height / 2) + 2):
                    for j in range(-int(self.period / 2), int(self.period / 2)):
                        if self.sw_ne_horizontal[axis_horizontal + i][axis_vertical + j] == 1:
                            if self.sw_ne_horizontal[axis_horizontal - i + 2][axis_vertical - j - 1] != 1:
                                flag = 1
                for i in range(-int(self.height / 2), int(self.height / 2) + 1):
                    for j in range(-int(self.period / 2), int(self.period / 2)):
                        if self.nw_se_horizontal[axis_horizontal + i][axis_vertical + j] == 1:
                            if self.nw_se_horizontal[axis_horizontal - i][axis_vertical - j - 1] != 1:
                                flag = 1
            ...
            for axis_vertical in range(int(self.period / 2) - 1, int(self.length / 2) + 1):
                ...
                for i in range(-int(self.height / 2) + 1, int(self.height / 2) + 1):
                    for j in range(-int(self.period / 2), int(self.period / 2) + 2):
                        if self.north_south_horizontal[axis_horizontal + i][axis_vertical + j] == 1:
                            if self.north_south_horizontal[axis_horizontal - i + 1][axis_vertical - j + 1] != 1:
                                flag = 1
                for i in range(-int(self.height / 2) + 1, int(self.height / 2) + 1):
                    for j in range(-int(self.period / 2), int(self.period / 2) + 1):
                        if self.sw_ne_horizontal[axis_horizontal + i][axis_vertical + j] == 1:
                            if self.sw_ne_horizontal[axis_horizontal - i + 1][axis_vertical - j] != 1:
                                flag = 1
                for i in range(-int(self.height / 2), int(self.height / 2)):
                    for j in range(-int(self.period / 2), int(self.period / 2) + 1):
                        if self.nw_se_horizontal[axis_horizontal + i][axis_vertical + j] == 1:
                            if self.nw_se_horizontal[axis_horizontal - i - 1][axis_vertical - j] != 1:
                                flag = 1
                if flag == 0:
                    return True
            for axis_vertical in range(int(self.period / 2) - 1, int(self.length / 2) + 1):
                ...
                for i in range(-int(self.height / 2), int(self.height / 2) + 1):
                    for j in range(-int(self.period / 2), int(self.period / 2)):
                        if self.west_east_horizontal[axis_horizontal + i][axis_vertical + j] == 1:
                            if self.west_east_horizontal[axis_horizontal - i][axis_vertical - j - 1] != 1:
                                flag = 1
                for i in range(-int(self.height / 2) + 1, int(self.height / 2) + 1):
                    for j in range(-int(self.period / 2), int(self.period / 2) + 1):
                        if self.north_south_horizontal[axis_horizontal + i][axis_vertical + j] == 1:
                            if self.north_south_horizontal[axis_horizontal - i + 1][axis_vertical - j] != 1:
                                flag = 1
                for i in range(-int(self.height / 2) + 1, int(self.height / 2) + 1):
                    for j in range(-int(self.period / 2), int(self.period / 2)):
                        if self.sw_ne_horizontal[axis_horizontal + i][axis_vertical + j] == 1:
                            if self.sw_ne_horizontal[axis_horizontal - i + 1][axis_vertical - j - 1] != 1:
                                flag = 1
                for i in range(-int(self.height / 2), int(self.height / 2)):
                    for j in range(-int(self.period / 2), int(self.period / 2)):
                        if self.nw_se_horizontal[axis_horizontal + i][axis_vertical + j] == 1:
                            if self.nw_se_horizontal[axis_horizontal - i - 1][axis_vertical - j - 1] != 1:
                                flag = 1
                if flag == 0:
                    return True
            return False
