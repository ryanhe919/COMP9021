...
class Frieze:
    ...
    def is_vertical_reflection(self):
        type = 0
        symmetry_axis = 0
        for axis in range(self.period):
            temp = 0
            for i in range(int(self.period / 2)):
                if self.north_south_vertical[axis - i] == self.north_south_vertical[axis + i + 1]:
                    temp += 1
        ...
        for axis in range(self.period):
            temp = 0
            for i in range(1, int(self.period / 2) + 1):
                if self.north_south_vertical[axis- i] == self.north_south_vertical[axis + i]:
                    temp += 1
            if temp == int(self.period / 2):
                symmetry_axis = axis
                type = 2
        ...
        if type == 1:
            temp = 0
            for i in range(1, int(self.period / 2)):
                if self.west_east_vertical[symmetry_axis - i] == self.west_east_vertical[symmetry_axis + i]:
                    temp += 1
            if temp != int(self.period / 2) - 1:
                return False
            for i in range(int(self.period / 2)):
                for j in range(len(self.sw_ne_vertical[symmetry_axis - i])):
                    if self.sw_ne_vertical[symmetry_axis - i][j] == 1:
                        if self.nw_se_vertical[symmetry_axis + i][j - 1] != 1:
                            return False
            for i in range(int(self.period / 2)):
                for j in range(len(self.nw_se_vertical[symmetry_axis - i])):
                    if self.nw_se_vertical[symmetry_axis - i][j] == 1:
                        if self.sw_ne_vertical[symmetry_axis + i][j + 1] != 1:
                            return False
        if type == 2:
            temp = 0
            for i in range(int(self.period / 2)):
                if self.west_east_vertical[symmetry_axis - i - 1] == self.west_east_vertical[symmetry_axis + i]:
                    temp += 1
            if temp != int(self.period / 2):
                return False
            for i in range(int(self.period / 2)):
                for j in range(len(self.sw_ne_vertical[symmetry_axis - i - 1])):
                    if self.sw_ne_vertical[symmetry_axis - i - 1][j] == 1:
                        if self.nw_se_vertical[symmetry_axis + i][j - 1] != 1:
                            return False
            for i in range(int(self.period / 2)):
                for j in range(len(self.nw_se_vertical[symmetry_axis - i - 1])):
                    if self.nw_se_vertical[symmetry_axis - i - 1][j] == 1:
                        if self.sw_ne_vertical[symmetry_axis + i][j + 1] != 1:
                            return False
    ...
    def is_horizontal_reflection(self):
        self.print_list(self.west_east_horizontal)
        if self.height % 2 == 1:
                ...
                for j in range(len(self.north_south_horizontal[axis - i])):
                    if self.north_south_horizontal[axis - i][j] == 1:
                        if self.north_south_horizontal[axis + i + 2][j] != 1:
                            return False
            for i in range(int(self.height / 2 + 1)):
                for j in range(len(self.sw_ne_horizontal[axis - i + 1])):
            ...
            axis = int(self.height / 2)
            for i in range(1, int(self.height / 2 + 1)):
                for j in range(len(self.west_east_horizontal[axis - i])):
                    if self.west_east_horizontal[axis - i][j] == 1:
                        if self.west_east_horizontal[axis + i][j] != 1:
                            return False
            for i in range(int(self.height / 2)):
                for j in range(len(self.north_south_horizontal[axis - i - 1])):
