...
class SortingExperimenter(tk.Tk):
    def __init__(self):
        ...
        possible_sorting_algorithms = 'Bubblesort', 'Selectionsort', 'Insertionsort',\
                                   'Shellsort', 'Mergesort', 'Quicksort', 'Heapsort', 'Batchersort'
    ...
    def blue_help(self):
        tkinter.messagebox.showinfo('Blue',
            ...
            'For recursive algorithms (Quicksort, Mergesort), the numbers in blue (together '
            'with or excepted those whose status is special and which receive a different colour) '
            'are those involved in the current recursive call.'
                                   )

    def red_help(self):
        tkinter.messagebox.showinfo('Red',
            'For all algorithms except Mergesort, two numbers are colored red when they are '
            'about to be swapped.\n\n'
            'For Mergesort, a number is coloured in red when it is about to be copied to a '
    ...
    def yellow_help(self):
        tkinter.messagebox.showinfo('Yellow',
            ...
            ' - or when a number has been copied from somewhere else (which except for Mergesort, '
            'means that it has been swapped with another number) and is not known to have been '
            'put in place.'
                                   )
    ...
    def green_help(self):
        tkinter.messagebox.showinfo('Green',
            ...
            'For Insertionsort and Shellsort, this happens at the very end.\n\n'
            'For Bubblesort, this can happen earlier.\n\n'
    ...
    def set_initialisation_interface(self, possible_sorting_algorithms):
        ...
        for i in range(2, 21):
            input_size.add_command(label = f'{i:2d}',
...
class ListToSort(tk.Frame):
    ...
    def display_list(self, length):
        ...
        self.displayed_list.create_line(self.offset, 5,
        ...
        while swapped and bound:
            ...
            for i in range(bound):
                if self.list[i] > self.list[i + 1]:
                    ...
                    self.displayed_list.itemconfig(self.cells[i + 1],
                    ...
                    yield
                    self.displayed_list.itemconfig(self.cells[i],
                                       fill = self.live_item_colour, font = ('normal', 0, 'normal')
                                                  )
                    self.displayed_list.itemconfig(self.cells[i + 1],
                                                                     font = ('normal', 0, 'normal')
                                                  )
        ...
        for i in range(self.length - 1):
            ...
            if index_of_min != i:
                ...
                if i == self.length - 2 or self.length == 2:
                    self.displayed_list.itemconfig(self.cells[index_of_min],
                                   text = self.list[index_of_min], fill = self.in_place_item_colour
                                                  )
                    yield
                else:
                    self.displayed_list.itemconfig(self.cells[index_of_min],
            ...
            for i in range(n, 2 * n):
                for j in range(i, self.length, n):
                    ...
                    while k >= n and self.list[k - n] > self.list[k]:
                        ...
                        self.displayed_list.itemconfig(self.cells[k],
                                                                    fill = self.moving_item_colour,
                                                                       font = ('normal', 0, 'bold')
                                                      )
                        yield
    ...
    def trace_merge_sort(self):
        ...
        yield from self.trace_mergesort(self.list, self.extra_list, 0, self.length, 0)
