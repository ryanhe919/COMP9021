...
def max_diff_in_consecutive_leaves(tree):
    list_leaves = []
    list_leaves = trans_to_list(tree, list_leaves)
    diff = []
    if len(list_leaves) == 0 or len(list_leaves) == 1:
        return 0
    for i in range(len(list_leaves) - 1):
        diff.append(abs(list_leaves[i + 1] - list_leaves[i]))
    return max(diff)
